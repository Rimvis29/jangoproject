...
const char * const BITCOIN_CONF_FILENAME = "jango.conf";
const char * const BITCOIN_PID_FILENAME = "jangod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "jango" is a composite category enabling all Jango-related debug output
            if(ptrCategory->count(string("jango"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JangoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JangoCore
    // Mac: ~/Library/Application Support/JangoCore
    // Unix: ~/.jangocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JangoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JangoCore";
#else
    // Unix
    return pathRet / ".jangocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty jango.conf if it does not excist
        ...
        if (configFile != NULL) {
        	...
        	LogPrintf("jango.conf file not found or can't be created\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override jango.conf
...
void AddSeedsToConfigFile(FILE* configFile) {
	...
	fprintf(configFile,"addnode=178.128.171.164:13058\r\n");
